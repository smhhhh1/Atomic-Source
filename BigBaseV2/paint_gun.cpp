#include "common.hpp"
#include "gta_util.hpp"
#include "paint_gun.h"
#include "natives.hpp"
#include "pointers.hpp"
#include "script.hpp"
#include "../render.h"
#include "../abstract_submenu.h"
#include "../break_option.h"
#include "../toggle_option.h"
#include "../number.h"
#include "../option.h"
#include "../submenu_option.h"
#include "../simple_option.h"
#include "../simple_submenu.h"
#include "../bool_and_number.h"
#include "../submenu_option.h"
#include "../submenus.h"
#include "on_screen_ped.h"
#include "lists.h"
#include "fiber_pool.hpp"
#include "scroll.h"
#include "util.h"
using namespace big::weapon::paint_gun::vars;

namespace big::weapon::paint_gun::vars {
	variables m_vars;

	void RGBFade()
	{
		if (m_vars.red > 0 && m_vars.blue == 0) {
			m_vars.red--;
			m_vars.green++;
		}
		if (m_vars.green > 0 && m_vars.red == 0) {
			m_vars.green--;
			m_vars.blue++;
		}
		if (m_vars.blue > 0 && m_vars.green == 0) {
			m_vars.red++;
			m_vars.blue--;
		}
	}

}

namespace big::menu
{


	void paint_gun_menu::menu()
	{
		using namespace ui;

		m_renderer.new_sub<sub>("Paint Gun", SubPaintGun, [](sub* core)
			{
				core->add_option<toggle_option<bool>>(("Toggle"),
					nullptr,
					&m_vars.m_toggle
				);
				core->add_option<toggle_option<bool>>(("Rainbow"),
					nullptr, 
					&m_vars.m_rainbow
				);
				core->add_option<number_option<float>>(("Intensity"),
					nullptr,
					&m_vars.m_intensity, 250.0f,
					1000.0f, 10,
					1, false
				);
				core->add_option<break_option>(std::move(break_option().
					SetRightText("Colors")
				));
				core->add_option<number_option<int>>(("Red"),
					nullptr, 
					&m_vars.red, 0, 
					255, 1,
					1, false
				);
				core->add_option<number_option<int>>(("Green"),
					nullptr,
					&m_vars.green,0,
					255, 1, 
					1, false
				);
				core->add_option<number_option<int>>(("Blue"),
					nullptr,
					&m_vars.blue, 0,
					255, 1,
					1, false
				);
			});
	}

	void paint_gun_menu::script_init() {}

	void paint_gun_menu::script_on_tick() {


		if (m_vars.m_toggle) {
			static std::vector<Vector3> a;
			Vector3 b;
			if (WEAPON::GET_PED_LAST_WEAPON_IMPACT_COORD(PLAYER::PLAYER_PED_ID(), &b));
			{
				a.push_back(b);
			}
			for (auto pos : a)
			{
				if (m_vars.m_rainbow) {
					RGBFade();
					GRAPHICS::DRAW_LIGHT_WITH_RANGE(pos.x, pos.y, pos.z, m_vars.red, m_vars.green, m_vars.blue, m_vars.m_range, m_vars.m_intensity);
				}
				else {
					GRAPHICS::DRAW_LIGHT_WITH_RANGE(pos.x, pos.y, pos.z, m_vars.red, m_vars.green, m_vars.blue, m_vars.m_range, m_vars.m_intensity);
				}

			}
		}

	}

	void paint_gun_menu::script_func()
	{
		m_paint_gun_menu.script_init();
		while (true)
		{
			m_paint_gun_menu.script_on_tick();
			script::get_current()->yield();
		}
	}
}