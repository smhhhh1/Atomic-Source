...
namespace big
{
	player_database_service::player_database_service() :
		m_file_path("C:\\Atomic\\Blacklist\\blacklist.json")
	{
		load();
	...
	void player_database_service::load()
	{
		...
		if (std::filesystem::exists(m_file_path))
		{
			...
			{
				m_players[std::stoi(p.key())] = p.value().get<blacklist_helper>();
			}
		}
	}

	std::unordered_map<std::uint64_t, blacklist_helper>& player_database_service::get_players()
	{
		return m_players;
	}

	blacklist_helper* player_database_service::get_player_by_rockstar_id(std::uint64_t rockstar_id)
	{
	...
	blacklist_helper* player_database_service::get_or_create_player(player_ptr player)
	{
	...
	void player_database_service::set_selected(blacklist_helper* selected)
	{
		m_selected = selected;
	}

	blacklist_helper* player_database_service::get_selected()
	{
		return m_selected;
	}
}