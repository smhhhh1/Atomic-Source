...
#define NOTIFY_PADDING_Y				950.f		// Bottom-left Y padding
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			SetNextWindowSize(ImVec2(NULL, 50), 0);
			...
			// Here we render the toast content
			{
				...
				for (int i = 0; i < height; i++)
				{
					...
					draw_list->AddRectFilled(ImVec2(panelPos.x, panelPos.y + i), ImVec2(panelPos.x + 5, panelPos.y + 4), ImU32(ImColor(86, 59, 236, 255)));
				}
				...
				ImGui::InvisibleButton("##123", ImVec2(1, panelPos.y), 0); ImGui::SameLine(); Text(content); ImGui::SameLine(); ImGui::InvisibleButton("##1234", ImVec2(1, panelPos.y), 0);
